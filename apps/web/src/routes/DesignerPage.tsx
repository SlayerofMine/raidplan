...
export function DesignerPage() {
  ...
  const save = useCallback(async () => {
    ...
    const def = planToAttack(
      useEditorStore.getState().getPlan(),
      {
        id: defId.current,
        name,
        // Authored inside a plan, so it belongs to that plan. Publishing one to
        // an encounter is a separate, admin-only act (§21.7).
        source: "plan",
      },
      params,
    );
    const next = upsertAttack(plan, def);
