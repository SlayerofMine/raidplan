...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-27 (commit 9cd3b82). Confidence: 100%.
...
- `apps/web/src/editor/canvas/CanvasStage.tsx` — 4 bug fixes, last fix today (bug magnet); 16 commits/90d
- `packages/shared/test/attack.test.ts` — 4 bug fixes, last fix today (bug magnet); 17 commits/90d
- `packages/shared/src/attack.ts` — 4 bug fixes, last fix today (bug magnet); 17 commits/90d
...
### Code health
Three co-equal signals: defect risk 8.48/10 avg, hotspot health 6.76/10 (stable), worst `apps/web/src/editor/canvas/AttackPreviewLayer.tsx` at 1.8/10 · maintainability 9.37/10 · performance risk 2 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `apps/web/src/anim/usePlayback.ts` — function hotspot (usePlayback) — impact −1.9
- `apps/web/test/anim/attackCollision.test.ts` — prior defect — impact −1.8
- `apps/web/src/anim/usePlayback.ts` — prior defect — impact −1.6
