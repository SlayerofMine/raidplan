...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-27 (commit 3a43d58). Confidence: 100%.
...
- `apps/web/src/editor/canvas/CanvasStage.tsx` — 4 bug fixes, last fix today (bug magnet); 17 commits/90d
- `packages/shared/test/attack.test.ts` — 4 bug fixes, last fix today (bug magnet); 18 commits/90d
- `packages/shared/src/attack.ts` — 4 bug fixes, last fix today (bug magnet); 18 commits/90d
- `apps/web/src/editor/canvas/ObjectNode.tsx` — 4 bug fixes, last fix 4 days ago (bug magnet); 16 commits/90d
- `apps/web/test/anim/attackCollision.test.ts` — 3 bug fixes, last fix 4 days ago; 7 commits/90d

### Code health
Three co-equal signals: defect risk 8.46/10 avg, hotspot health 6.89/10 (stable), worst `packages/shared/src/attack.ts` at 2.77/10 · maintainability 9.39/10 · performance risk 3 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `apps/api/src/db/schema.ts` — untested hotspot — impact −2.0
- `apps/web/src/anim/usePlayback.ts` — function hotspot (usePlayback) — impact −1.7
