...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-28 (commit 5bac40b). Confidence: 100%.
...
- `apps/web/src/editor/canvas/CanvasStage.tsx` — 4 bug fixes, last fix today (bug magnet); 19 commits/90d
...
### Code health
Three co-equal signals: defect risk 8.11/10 avg, hotspot health 6.19/10 (stable), worst `apps/web/src/editor/canvas/AttackPreviewLayer.tsx` at 1.8/10 · maintainability 9.36/10 · performance risk 3 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `apps/web/src/editor/canvas/ObjectNode.tsx` — prior defect — impact −1.4
- `apps/web/src/anim/usePlayback.ts` — function hotspot (usePlayback) — impact −1.1
- `apps/web/src/editor/canvas/CanvasStage.tsx` — prior defect — impact −1.1
- `apps/web/test/anim/attackCollision.test.ts` — prior defect — impact −1.1
- `apps/web/src/store/editorStore.ts` — function hotspot (immer callback) — impact −1.1
