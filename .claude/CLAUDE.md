...
Even without LLM access for Repowise, the MCP is completely usable, not degraded.
...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-27 (commit 814d729). Confidence: 100%.
...
- `apps/web/src/editor/canvas/CanvasStage.tsx` — 4 bug fixes, last fix today (bug magnet); 18 commits/90d
- `packages/shared/test/attack.test.ts` — 4 bug fixes, last fix today (bug magnet); 19 commits/90d
- `packages/shared/src/attack.ts` — 4 bug fixes, last fix today (bug magnet); 19 commits/90d
- `apps/web/src/editor/canvas/ObjectNode.tsx` — 4 bug fixes, last fix 5 days ago (bug magnet); 16 commits/90d
- `apps/web/test/anim/attackCollision.test.ts` — 3 bug fixes, last fix 5 days ago; 8 commits/90d

### Code health
Three co-equal signals: defect risk 8.29/10 avg, hotspot health 6.64/10 (stable), worst `apps/web/src/editor/canvas/AttackPreviewLayer.tsx` at 1.8/10 · maintainability 9.35/10 · performance risk 3 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `apps/web/src/anim/usePlayback.ts` — function hotspot (usePlayback) — impact −1.3
- `apps/web/src/editor/canvas/ObjectNode.tsx` — prior defect — impact −1.3
- `apps/web/src/store/editorStore.ts` — function hotspot (immer callback) — impact −1.2
- `apps/web/test/anim/attackPlayback.test.ts` — hidden coupling — impact −1.0
- `apps/api/test/attacks/attacksRepo.test.ts` — hidden coupling — impact −1.0
